...
mdat<-mdat %>% filter(Animal %in% c("131132","131130","131115","131116","131111","154187"))
...
```{r}
byhourall<-state_est %>% group_by(LocalHour,Month,Behavior) %>% summarize(n=n()) %>% mutate(prop=n/sum(n))
byhourall$Month<-factor(byhourall$Month,levels=month.name)
ggplot(byhourall,aes(x=LocalHour,y=prop,col=Behavior)) + geom_line()  + geom_point() + theme_bw() + scale_y_continuous("Frequency of Behavior",labels=scales::percent)
