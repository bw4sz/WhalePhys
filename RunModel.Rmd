...
mdat<-mdat %>% select(ID,Animal,Latitude,Longitude,timestamp,Date,Month,Hour,Year,DepthMax,DurationMax,DurationMin,Quality) 
...
#paramters to track
pt<-c("alpha","lambda_count","duration_mu","duration_tau","depth_mu","depth_tau","dive_new","state","E","Enew")
...
#single index
splitpc[c("alpha","depth_mu","depth_tau","duration_mu","duration_tau","lambda_count")]<-lapply(splitpc[c("alpha","depth_mu","depth_tau","duration_mu","duration_tau","lambda_count")],function(x){
...
```
##Distribution of average max dive durations
...
#simulate dives 
travel_dives<-data.frame(Maxduration=rtruncnorm(statecount[1,"n"], mean=duration_mu1$value,sd=sqrt(1/duration_tau1$value),a=0),Behavior="Traveling")
forage_dives<-data.frame(Maxduration=rtruncnorm(statecount[2,"n"], mean=duration_mu2$value,sd=sqrt(1/duration_tau2$value),a=0),Behavior="Foraging")
pred_dives<-bind_rows(travel_dives,forage_dives)

ggplot(pred_dives)  + geom_histogram(data=mdat,aes(x=DurationMax),col="black")+ geom_histogram(alpha=0.8,aes(x=Maxduration,fill=Behavior))  + theme_bw() 

ggplot(pc_dive[pc_dive$parameter=="dive_new",])+  geom_density(aes(x=value),col="red") + geom_density(data=mdat,aes(x=DurationMax),col="black",size=1.5) + theme_bw() 
```

##Distribution of average dive counts per step
...
```{r}
ggplot(state_est,aes(x=timestamp,y=-DepthMax,col=Behavior)) + geom_point() + geom_line(size=0.25) + facet_wrap(~Animal,scales="free") + theme_bw()
