...
mdat<-mdat %>% filter(Animal %in% c("154187","131115","131116","131111","131134"))
...
if(newModel){
  system.time(diving<-jags.parallel(model.file = "Bayesian/NestedDive.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=1000,n.burnin=900,n.thin=2,DIC=FALSE,inits = InitStage))
  }
...
#single index
splitpc[c("sub_alpha","alpha","gamma")]<-lapply(splitpc[c("sub_alpha","alpha","gamma")],function(x){
...
#double index
splitpc[c("depth_mu","depth_tau")]<-lapply(splitpc[c("depth_mu","depth_tau")],function(x){
...
if(newModel){
  system.time(foraging<-jags.parallel(model.file = "Bayesian/Foraging.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=10000,n.burnin=9000,n.thin=2,DIC=FALSE))
  }
...
```{r}
allmodels<- allmodels %>% dplyr::select(-c(sub_state,state)) 
...
```{r}
allmodels<-allmodels %>% filter(!is.na(`3D`))
