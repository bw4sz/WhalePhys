...
if(newModel){
  system.time(diving<-jags.parallel(model.file = "Bayesian/Diving.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=20000,n.burnin=19500,n.thin=2,DIC=FALSE))
  }
...
ggplot(divecov,aes(x=DepthMax*1000,y=mean)) + geom_ribbon(aes(ymax=upper,ymin=lower),alpha=0.5)+ geom_line(linetype="dashed",col="red") +  theme_bw() + labs(x="Dive Depth (m)",y="Probability of Traveling")
```
...
```{r}
ggplot(data=allmodels)+geom_path(aes(x=Longitude, y=Latitude,group=Animal),size=.5) + geom_point(aes(x=Longitude, y=Latitude,col=Behavior))+ borders(fill="grey90") + coord_cartesian(ylim = range(mdat$Latitude),xlim=range(mdat$Longitude)) + theme_bw() + mytheme  + facet_wrap(~Model)
```
...
ggplot(allmodels)+geom_path(aes(x=Longitude, y=Latitude,group=Animal),size=.5) + geom_point(aes(x=Longitude, y=Latitude,col=Mismatch))+ borders(fill="grey90") + coord_cartesian(ylim = range(mdat$Latitude),xlim=range(mdat$Longitude)) + theme_bw() + mytheme + scale_color_manual(values=c("Black","Red"))
```
