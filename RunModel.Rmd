...
#paramters to track
pt<-c("alpha","depth_mu","depth_tau","dive_new","state","lambda_count","E","Enew")

if(newModel){
  system.time(diving<-jags.parallel(model.file = "Bayesian/Diving.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=7000,n.burnin=6500,n.thin=2,DIC=FALSE))
  }
...
```{r,eval=F}
predframe<-pc_dive %>% filter(parameter=="dive_new") %>% mutate(Animal=as.numeric(as.character(Animal)),Track=as.numeric(as.character(Track)),jStep=as.numeric(as.character(jStep))) %>% inner_join(mdat,by=c("Animal","Track","step","jStep")) %>% select(Animal,Latitude,Longitude,Track,step,jStep,timestamp,Predicted=value,Observed=DepthMax)
...
#relevel
predframe[predframe$state==1,"state"]<-"Traveling"
predframe[predframe$state==2,"state"]<-"Foraging"

ggplot(data=predframe) + geom_density(aes(x=DepthMax,fill=state),alpha=0.8)  + theme_bw() + facet_wrap(~variable,nrow=2)

ggplot(data=predframe) + geom_density(aes(x=DepthMax,fill=variable),alpha=0.8)  + theme_bw() + facet_wrap(~state,nrow=2,scale="free")+ ggtitle("By Behavior")

ggplot(data=predframe) + geom_density(aes(x=DepthMax,fill=variable),alpha=0.8)  + theme_bw() + facet_grid(Animal~state,scale="free")+ ggtitle("By Behavior and Animal")
```
