...
mdat<-mdat %>% filter(Animal %in% c("131136","131115"))
...
if(newModel){
  system.time(diving<-jags.parallel(model.file = "Bayesian/TimeDive.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=10000,n.burnin=9000,n.thin=2,DIC=FALSE,inits = InitStage))
  }
...
for(i in 1:nrow(statecount)){
  ...
  for(x in 1:length(depth_mu)){
      dive_intercept<-rtruncnorm(n=1,mean=depth_mu$value[x],sd=1/sqrt(depth_tau$value[x]),a=0)
       DepthMax<-dive_intercept  + beta$value[x]* cos((2*pi*unique(mdat$LocalHour))/(24)) + beta2$value[x] * sin((2*pi*unique(mdat$LocalHour))/24)^2
       pred_depths[[x]]<-data.frame(DepthMax,Hour=unique(mdat$LocalHour),Behavior=statecount[i,"Behavior"])
  }
...
ggplot(pred_dives) + geom_histogram(alpha=0.8,aes(x=DepthMax*1000,fill=Behavior))  + theme_bw() + labs(x="Dive Depth (m)")
ggsave("Figures/DiveHist.jpg",height=4,width=7)

#By hours
sumhours<-pred_dives %>% group_by(Hour,Behavior) %>% summarize(mean=mean(DepthMax)*1000,upper=quantile(DepthMax*1000,0.95),lower=quantile(DepthMax*1000,0.05)) 
...
if(newModel){
  system.time(foraging<-jags.parallel(model.file = "Bayesian/Foraging.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=20000,n.burnin=19500,n.thin=2,DIC=FALSE))
  }
