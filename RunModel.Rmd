...
mdat<-mdat %>% filter(Animal %in% c("131132","131130","131115","131116","154187","131133"))
...
if(newModel){
  system.time(diving<-jags.parallel(model.file = "Bayesian/NestedDive.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=10000,n.burnin=9500,n.thin=2,DIC=FALSE,inits = InitStage))
  }
...
ggplot(state_est %>% filter(!is.na(DepthMax)),aes(x=timestamp,y=-DepthMax,col=Behavior,group=Track)) + geom_point() + geom_line(size=0.25,aes(group=Track)) + facet_wrap(~Animal,scales="free",ncol=1) + theme_bw()
...
```{r}
ggmap(troy) +geom_path(data=allmodels,aes(x=Longitude, y=Latitude,group=paste(Animal,Track),col=Behavior),size=0.5) + geom_point(data=allmodels,aes(x=Longitude, y=Latitude,group=1,col=Behavior),size=0.3) + mytheme + facet_wrap(~Model)
