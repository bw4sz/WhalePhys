...
ggplot()+geom_path(data=mxy, aes(x=x, y=y,group=Animal),size=.5)  + borders(fill="grey90") + coord_cartesian(ylim = range(mxy$y),xlim=range(mxy$x)) + theme_bw() + mytheme  + scale_color_discrete(guide="none")
...
```{r,child="Bayesian/Foraging.R",eval=T}
```
...
if(newModel){
  system.time(jagM<-jags.parallel(model.file = "Bayesian/Foraging.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=20000,n.burnin=19000,n.thin=2,DIC=FALSE))
}
...
##Dates don't quite match
gdat %>% filter(Ptt=="131111")
mdat %>% filter(Animal=="131111",ID==1)
messages %>% filter(Ptt=="131111",ID==1)
...
#paramters to track
pt<-c("alpha","depth_mu","depth_tau","dive_new","state")

if(newModel){
  system.time(jagM<-jags.parallel(model.file = "Bayesian/Diving.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=5000,n.burnin=4000,n.thin=2,DIC=FALSE))
}
...
#single index
splitpc[c("alpha","depth_mu","depth_tau")]<-lapply(splitpc[c("alpha","depth_mu","depth_tau")],function(x){
    sv<-data.frame(str_match(x$par,"(\\w+)\\[(\\d+)]"))[,3]
    pc<-data.frame(x,Behavior=sv)
    return(pc)
}) 
...
#3 index
splitpc[c("state")]<-lapply(splitpc[c("dive_new")],function(x){
    sv<-data.frame(str_match(x$par,"(\\w+)\\[(\\d+),(\\d+),(\\d+),(\\d+)]"))[,3:5]
...
predframe<-pred_dives %>% inner_join(mdat,by=c("Animal","Track","step","jStep")) %>% select(Animal,Latitude,Longitude,Track,step,jStep,timestamp,Pred_DepthMax,DepthMax)

#merge with state
pc_dive %>% filter(parameter=="state") %>% group_by(Animal,Track,step,jStep) %>% summarize(state=names(sort(table(value)))[1]) %>% ungroup()  %>% mutate(Animal=as.numeric(as.character(Animal)),Track=as.numeric(as.character(Track)),jStep=as.numeric(as.character(jStep))) %>% inner_join(predframe,by=c("Animal","Track","step"))

ggplot(data=predframe) + geom_point(aes(x=DepthMax,y=Pred_DepthMax)) + geom_abline(linetype="dashed") + 

ggplot(data=predframe) + geom_point(aes(x=DepthMax,y=Pred_DepthMax)) + facet_wrap(~Animal) + theme_bw()

ggplot(data=predframe) + geom_point(aes(x=DepthMax,y=Pred_DepthMax)) + facet_wrap(~state) + theme_bw()
ggplot(data=predframe) + geom_point(aes(x=DepthMax,y=Pred_DepthMax)) + facet_wrap(~Animal) + theme_bw()
...
chisquared<-function(o,e){(o-e)^2/e}
chiframe<-predframe %>% group_by(Animal,Track,step,jStep) %>% do(chi=chisquared(o=.$Pred_DepthMax,e=.$DepthMax)) 

ggplot(chiframe,aes(x=DepthMax,y=chi)) + geom_point()

ggplot(chiframe,aes(x=as.factor(Animal),y=chi)) + geom_boxplot() + labs(x="Animal")
```
