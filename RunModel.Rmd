...
mdat<-mdat %>% select(ID,Animal,Latitude,Longitude,timestamp,Date,Month,Hour,Year,DepthMax,DurationMax,Quality) 
...
#Calculate dive "speed" Seconds/max depth
mdat$DiveSpeed<-mdat$DepthMax*1000/mdat$DurationMax
...
divespeed<-reshape2::acast(mdat,Animal~Track~step,value.var="DiveSpeed",fun.aggregate = length)
...
#paramters to track
pt<-c("alpha","depth_mu","depth_tau","dive_new","state","E","Enew")

if(newModel){
  system.time(diving<-jags.parallel(model.file = "Bayesian/Diving.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=100000,n.burnin=99000,n.thin=2,DIC=FALSE))
  }
