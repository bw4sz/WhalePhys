...
ggplot(mdat,aes(x=DurationMax,y=DepthMax)) + geom_point() + theme_bw() + labs(x="Duration (min)",y="Depth (m)")

ggplot(mdat %>% filter(DepthMax<50),aes(x=DurationMax,y=DepthMax)) + geom_point() + theme_bw() + labs(x="Duration (min)",y="Depth (m)")
...
if(newModel){
  system.time(diving<-jags.parallel(model.file = "Bayesian/ThreeState.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=2000,n.burnin=1900,n.thin=2,DIC=FALSE))
  }
...
#argos index
splitpc[c("argos")]<-lapply(splitpc[c("argos")],function(x){
    sv<-data.frame(str_match(x$par,"(\\w+)\\[(\\d+),(\\d+),(\\d+),(\\d+),(\\d+)]"))[,3:7]
    colnames(sv)<-c("Animal","Track","step","jStep","xy")
    pc<-data.frame(x,sv)
    return(pc)
}) 
...
statecount<-pc_dive %>% filter(parameter=="state") %>% group_by(jAnimal=Animal,Track,step) %>% summarize(state=names(sort(table(value)))[1]) %>% ungroup() %>% mutate(jAnimal=as.numeric(as.character(jAnimal)),Track=as.numeric(as.character(Track)),step=as.numeric(as.character(step)))  %>% inner_join(mdat,by=c("jAnimal","Track","step")) %>% group_by(state) %>% summarize(n=n()) %>% data.frame()
...
```{r}
argos_est<-pc_dive %>% group_by(jAnimal=Animal,Track,step,jStep,xy) %>%  filter(parameter=="argos") %>% summarize(mean=mean(value),upper=quantile(value,0.95),lower=quantile(value,0.05)) %>% ungroup()  %>% mutate(jAnimal=as.numeric(as.character(jAnimal)),Track=as.numeric(as.character(Track)),step=as.numeric(as.character(step)),jStep=as.numeric(as.character(jStep))) %>% gather("position","value",-(jAnimal:xy)) %>% mutate(position=paste(position,xy,sep="_")) %>% dplyr::select(-xy) %>% spread(position,value) %>% inner_join(state_est)
```
...
```{r,fig.height=10,fig.width=10}
argos_est<-argos_est %>% arrange(Animal,Track,timestamp)
ggmap(troy) +geom_path(data=argos_est,aes(x=mean_1, y=mean_2,group=paste(Animal,Track)),size=0.5) + geom_point(data=argos_est,aes(x=mean_1, y=mean_2,col=Behavior),size=1) + mytheme
ggmap(troy) + geom_point(data=argos_est,aes(x=mean_1, y=mean_2,col=Behavior),size=3) + mytheme
```
...
```{r,fig.height=10,fig.width=10}
argos_est<-argos_est %>% arrange(Animal,Track,timestamp)
ggmap(troy) +geom_path(data=argos_est,aes(x=mean_1, y=mean_2,group=paste(Animal,Track)),size=0.5) +geom_path(data=argos_est,aes(x=lower_1, y=lower_2,group=paste(Animal,Track)),size=0.5) +geom_path(data=argos_est,aes(x=upper_1, y=upper_2,group=paste(Animal,Track)),size=0.5) + mytheme
```
...
if(newModel){
  system.time(foraging<-jags.parallel(model.file = "Bayesian/Foraging.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=100,n.burnin=0,n.thin=2,DIC=FALSE))
  }
