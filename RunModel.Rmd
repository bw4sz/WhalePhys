...
#sum table
pc_dive %>% filter(!parameter %in% c("dive_new","state","E","Enew","eval","argos")) %>% group_by(parameter) %>%  summarize(mean=mean(value),upper=quantile(value,0.95),lower=quantile(value,0.05)) 
...
ggplot(pc_dive[pc_dive$parameter=="dive_new",])+  geom_density(aes(x=value*1000),col='red') + geom_density(data=mdat,aes(x=DepthMax*1000),col="black",size=1.5) + theme_bw() + labs(x="Dive Depth (m)")  + facet_wrap(~Animal)
