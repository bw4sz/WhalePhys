...
mdat<-mdat %>% filter(Animal %in% c("131132","131115","131134","154187"))
...
#paramters to track
pt<-c("alpha","gamma","depth_mu","depth_tau","dive_new","state", "sub_state","E","Enew")

if(newModel){
  system.time(diving<-jags.parallel(model.file = "Bayesian/NestedDive.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=30000,n.burnin=29000,n.thin=2,DIC=FALSE,inits = InitStage))
  }
...
#3 index
splitpc[c("state","sub_state")]<-lapply(splitpc[c("state","sub_state")],function(x){
...
#sum table
pc_dive %>% filter(!parameter %in% c("dive_new","state","E","Enew","eval","sub_state")) %>% group_by(parameter,Behavior) %>% summarize(mean=round(mean(value),3),upper=round(quantile(value,0.95),3),lower=round(quantile(value,0.05),3)) 
...
if(newModel){
  system.time(foraging<-jags.parallel(model.file = "Bayesian/Foraging.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=30000,n.burnin=29000,n.thin=2,DIC=FALSE))
  }
