sink("Bayesian/NestedDiveGamma.jags")
cat("
    model{
    ...
    #TODO autocorrelation on a per dive basis
    sub_phi[i,g,t,u,1] <- sub_alpha[state[i,g,t]] 
    sub_phi[i,g,t,u,2] <- 1-sub_phi[i,g,t,u,1]
    sub_state[i,g,t,u] ~ dcat(sub_phi[i,g,t,u,])
    
    #for each dive depth
    #dive depth at time t
    divedepth[i,g,t,u] ~ dgamma(depth_mu[state[i,g,t],sub_state[i,g,t,u]],depth_tau[state[i,g,t],sub_state[i,g,t,u]])
    
    #Assess Model Fit
    
    #Fit dive discrepancy statistics
    eval[i,g,t,u] ~ dgamma(depth_mu[state[i,g,t],sub_state[i,g,t,u]],depth_tau[state[i,g,t],sub_state[i,g,t,u]])
    E[i,g,t,u]<-pow((divedepth[i,g,t,u]-eval[i,g,t,u]),2)/(eval[i,g,t,u])
    
    dive_new[i,g,t,u] ~ dgamma(depth_mu[state[i,g,t],sub_state[i,g,t,u]],depth_tau[state[i,g,t],sub_state[i,g,t,u]])
    ...
    #Dive Priors
    #Foraging dives
    depth_mu[2,1]  ~ dgamma(0.001,0.001)
    depth_tau[2,1] ~ dgamma(0.001,0.001)
    
    #Resting Dives
    depth_mu[2,2] ~ dgamma(0.001,0.001)
    depth_tau[2,2] ~ dgamma(0.001,0.001)
    
    #Traveling Dives
    depth_mu[1,1] ~ dgamma(0.001,0.001)
    depth_tau[1,1] ~ dgamma(0.001,0.001) 
