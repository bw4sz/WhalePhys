...
cat("
    ...
    #Substate
    sub_phi[i,g,t,1] <- sub_alpha[state[i,g,t]] 
    sub_phi[i,g,t,2] <- 1-sub_phi[i,g,t,1]
    sub_state[i,g,t] ~ dcat(sub_phi[i,g,t,])
    ...
    #Final Substate
    sub_phi[i,g,steps[i,g],1] <- sub_alpha[state[i,g,steps[i,g]]] 
    sub_phi[i,g,steps[i,g],2] <- 1-sub_phi[i,g,steps[i,g],1]
    sub_state[i,g,steps[i,g]] ~ dcat(sub_phi[i,g,steps[i,g],])
    ...
    divedepth[i,g,t,u] ~ dnorm(depth_mu[state[i,g,t],sub_state[i,g,t]],depth_tau[state[i,g,t],sub_state[i,g,t]])T(0,)
    
    #Assess Model Fit
    
    #Fit dive discrepancy statistics
    eval[i,g,t,u] ~ dnorm(depth_mu[state[i,g,t],sub_state[i,g,t]],depth_tau[state[i,g,t],sub_state[i,g,t]])T(0,)
    E[i,g,t,u]<-pow((divedepth[i,g,t,u]-eval[i,g,t,u]),2)/(eval[i,g,t,u])
    
    dive_new[i,g,t,u] ~ dnorm(depth_mu[state[i,g,t],sub_state[i,g,t]],depth_tau[state[i,g,t],sub_state[i,g,t]])T(0,)
    ...
    depth_mu[2,1] ~ dnorm(150,0.001)T(0,)
    depth_sigma[1] ~ dunif(0,100)
    depth_tau[2,1] <- 1/pow(depth_sigma[1],2) 

    #Resting Dives
    depth_mu[2,2] ~ dnorm(20,0.01)T(0,)
    depth_sigma[2] ~ dunif(0,5)
    depth_tau[2,2] <- 1/pow(depth_sigma[2],2) 

    #Traveling Dives
    depth_mu[1,1] ~ dunif(0,30)
    depth_tau[1,1] <- 0.01
