...
cat("
    ...
    phi[i,g,t,3] <- alpha_hours[state[i,g,t-1]] + beta_hours[state[i,g,t-1]]* cos((2*pi*hours[i,g,t])/(24)) + beta2_hours[state[i,g,t-1]] * sin((2*pi*hours[i,g,t])/24)^2
    ...
    phi[i,g,steps[i,g],3] <- alpha_hours[state[i,g,steps[i,g]-1]] + beta_hours[state[i,g,steps[i,g]-1]]* cos((2*pi*hours[i,g,steps[i,g]])/(24)) + beta2_hours[state[i,g,steps[i,g]-1]] * sin((2*pi*hours[i,g,steps[i,g]])/24)^2
    ...
    alpha_hours[1] ~ dnorm(0,0.0001)
    alpha_hours[2] ~ dnorm(0,0.0001)
    alpha_hours[3] ~ dnorm(0,0.0001)

    #Effect of time of day on transitioning to resting
    beta_hours[1] <- 0 
    beta_hours[2] ~ dnorm(0,0.0001)
    beta_hours[3] ~ dnorm(0,0.0001)

    #Transition from travel to resting
    beta2_hours[1] <- 0
    beta2_hours[2] ~ dnorm(0,0.0001)
    beta2_hours[3] ~ dnorm(0,0.0001)
