...
Metabolic rate while foraging ($\m_{foraging}$) = 4.14KJ/sec, Tyson et al. (2016),Williams et al. (2001).
...
pars<-list(
  iterations=10,## run simulation loop
...
traveling_state<-function(pars){
    ...
    #Cost of traveling
    energy<-pars$metabolic_traveling * pars$t_interval ##metabolic cost of traveling 
...
simulation<-function(pars){
  ...
  return(data.frame(ID=simid,step=1:length(states),state=states,energy=energy))
}
```
